if (!mp_grid_path(global.grid, path, x, y, obj_player.x, obj_player.y, false)) {
    ...
    if (_forwardX != 0 && _forwardY != 0) {
        ...
        if (angle > 0 && angle < 90) {
            ...
            if (collision_point(x + sprite_width/2, y + sprite_height/2 - 32, obj_wall, false, true)) path_insert_point(path, 1, path_get_point_x(path, 1) - 32, path_get_point_y(path, 1) + 32, 1);
